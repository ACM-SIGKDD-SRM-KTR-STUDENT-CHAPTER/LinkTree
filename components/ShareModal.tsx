...
const ShareModal: React.FC<ShareModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleShare = (platform: string) => {
    ...
    const message = encodeURIComponent("Check out SRMSIGKDD's Linktree at ");
    let shareUrl = '';
  ...
  return (
    <div className="fixed marb inset-0 grid flex items-center justify-center z-50 bg-black bg-opacity-50">
      <motion.div
        className="cardscaler relative flex flex-col items-center rounded-lg border-2 border-srmsigkdd2 bg-srmsigkdd text-srmsigkdd2 backdrop-blur-lg p-6 z-50 max-w-md mx-auto transition-transform duration-300 ease-in-out transform hover:scale-105"
        initial="hidden"
        animate="visible"
        exit="exit"
        variants={{
      ...
      >
        <header className="flex w-full items-center justify-between mb-4">
          <h2 className="font-semibold text-lg">Share Our Linktree</h2>
          <button
            className="flex justify-center items-center rounded-md p-2 hover:bg-srmsigkdd focus:outline-none focus-visible:ring-2 focus-visible:ring-srmsigkdd"
            onClick={onClose}
            aria-label="Close"
          >
        ...
        <motion.div className="mb-4" initial="hidden" animate="visible" variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0 } }} transition={{ duration: 1 }}>
          <Image width={400} height={150} className="inline-flex rounded-lg object-cover" src="/srmsigkdd.jpg" alt="Share Linktree Preview" />
        </motion.div>
        <div className="relative flex items-center w-full mb-4">
          ...
          >
            {displayedLinks.map((link, index) => (
              <div key={index} className="flex flex-col items-center justify-center gap-2 px-3 py-2 border-2 border-srmsigkdd2 bg-srmsigkdd2 text-srmsigkdd rounded-md cursor-pointer hover:bg-srmsigkdd hover:srmsigkdd hover:text-srmsigkdd2 transition duration-300 ease-in-out" onClick={() => handleShare(link.handler)}>
                <div className="bg-srmsigkdd flex h-12 w-12 items-center justify-center rounded-xl">
                  <FontAwesomeIcon icon={link.icon} className="h-6 w-6 text-srmsigkdd2" />
                </div>
        ...
        <div className="flex flex-col gap-4 w-full" data-testid="ShareModal-ShareList">
          <button
            className="flex flex-col items-center justify-center gap-2 px-3 py-2 border-2 border-srmsigkdd2 bg-srmsigkdd2 text-srmsigkdd rounded-md hover:bg-srmsigkdd hover:border-srmsigkdd2 hover:text-srmsigkdd2 transition duration-300 ease-in-out"
            onClick={handleCopy}
          >
            <div className="bg-srmsigkdd hover:border-srmsigkdd2 flex h-12 w-12 items-center justify-center rounded-xl">
              <FontAwesomeIcon icon={faClipboard} className="h-6 w-6 text-srmsigkdd2" />
            </div>
      ...
      {toastVisible && (
        <div className="fade fixed bottom-5 left-1/2 transform -translate-x-1/2 p-4 bg-white text-srmsigkdd rounded-lg shadow-md transition-opacity duration-300">
          <FontAwesomeIcon icon={faCheckCircle} className="mr-2 text-srmsigkdd" />Link copied!
        </div>
      )}
    </div>
  );
